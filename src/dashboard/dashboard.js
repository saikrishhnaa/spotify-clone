...
const loadPlaylist = async (endpoint, elementId) => {
  ...
  for (let { name, description, images, id } of items) {
    ...
    playlistItem.className =
      "bg-black-secondary rounded border-solid p-4 hover:cursor-pointer hover:bg-light-black";
    playlistItem.id = id;
...
const findCurrentTrack = () => {
  ...
  if (trackId) {
    ...
    const curentTrackIndex = loadedTracks?.findIndex(
      (trk) => trk.id === trackId,
    );
    return { curentTrackIndex, tracks: loadedTracks };
  }
  return null;
};

const playNextTrack = () => {
  const { curentTrackIndex = -1, tracks = null } = findCurrentTrack() ?? {};
  if (curentTrackIndex > -1 && curentTrackIndex < tracks.length - 1) {
    onPlayTrack(null, tracks[curentTrackIndex + 1]);
  }
};
...
const loadPlaylistTracks = ({ tracks }) => {
  ...
  for (let trackitem of tracks.items.filter((item) => item.track.preview_url)) {
    ...
    track.id = id;
    track.className =
      "track p-1 grid grid-cols-[50px_1fr_1fr_50px] items-center justify-items-start gap-4 rounded-md text-secondary hover:bg-light-black";
...
const fillContentForPlaylist = async (playlistId) => {
  ...
  coverElement.innerHTML = `<img class="h-36 w-36 object-contain" src="${images[0].url}" alt="" />
  <section>
...
const loadUserPlaylists = async () => {
  ...
  for (let { name, id } of playlists.items) {
    ...
    li.addEventListener("click", () => userPlaylistClick(id));
  ...
  audio.addEventListener("play", () => {
    ...
    progressInterval = setInterval(() => {
      ...
      songDurationCompleted.textContent = `${
        (audio.currentTime.toFixed(0) < 10 ? "0:0" : "0:") +
